...
def _macos_build_arch() -> str | None:
    ...
    if not archs:
        ...
        if cibw_archs and cibw_archs not in {"auto", "auto64", "native"}:
            archs = set(cibw_archs.split())

    if archs == {"arm64", "x86_64"}:
        return "universal2"
    if len(archs) == 1:
        arch = next(iter(archs))
...
def _normalize_macos_platform_tag(platform_tag: str) -> str:
    if sys.platform != "darwin" or not platform_tag.endswith("_universal2"):
        return platform_tag

    arch = _macos_build_arch()
    if arch is None or arch == "universal2":
        return platform_tag

    return f"{platform_tag.rsplit('_', 1)[0]}_{arch}"
...
def _build_native_runtime(build_temp: Path) -> list[Path]:
    if not (REPO_ROOT / "native" / "wfloat-core").exists():
        raise RuntimeError(
            ...
            "Build wfloat wheels from a full wfloat repository checkout."
        )
...
if _bdist_wheel is not None:
    ...
    class bdist_wheel(_bdist_wheel):
        ...
        def get_tag(self) -> tuple[str, str, str]:
            ...
            return "py3", "none", _normalize_macos_platform_tag(platform_tag)
