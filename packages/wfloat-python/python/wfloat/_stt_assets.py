...
from ._assets import SttModelAssets
from ._download import download_file, verify_checksum


@dataclass(frozen=True)
class CachedSttAssets:
    model_name: str
    family: str
    cache_dir: Path
    ...
    def require(self, key: str) -> Path:
        ...
        if path is None:
            raise ValueError(f"Missing required STT asset: {key}")
        return path
...
def _materialize_asset(
    ...
    *,
    ...
    if expected_checksum is not None and not verify_checksum(destination, expected_checksum):
        raise RuntimeError(f"Cached STT asset checksum mismatch for {destination}.")

    return destination


def cache_stt_assets(
    model_name: str,
    *,
    family: str,
    ...
    for key, source in sources.items():
        ...
        if not filename:
            raise ValueError(f"Could not derive filename for STT asset {key}.")
    ...
    return cache_stt_assets(
        ...
        checksums={
            ...
            for key, value in {
                ...
                "tokens": assets.tokens_checksum,
                "preprocessor": assets.preprocessor_checksum,
                "encoder": assets.encoder_checksum,
                "decoder": assets.decoder_checksum,
