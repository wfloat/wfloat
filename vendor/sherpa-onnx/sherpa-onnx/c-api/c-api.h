...
/** @brief Configuration for a Wfloat TTS model. */
typedef struct SherpaOnnxOfflineTtsWfloatModelConfig {
  /** Path to the Wfloat ONNX model. */
  const char *model;
...
/**
 * @brief Prepare Wfloat text for synthesis.
 *
 * The returned JSON contains aligned `text`, `text_clean`, and
 * `text_phonemes` arrays. The caller must release it with
...
/** @brief Release text returned by SherpaOnnxOfflineTtsWfloatPrepareText(). */
