// sherpa-onnx/csrc/offline-tts-wfloat-model.cc
//
...
class OfflineTtsWfloatModel::Impl {
 public:
  explicit Impl(const OfflineTtsModelConfig &config)
      : config_(config),
        ...
        allocator_{} {
    auto buf = ReadFile(config.wfloat.model);
  ...
  Impl(Manager *mgr, const OfflineTtsModelConfig &config)
      : config_(config),
        ...
        sess_opts_(GetSessionOptions(config)),
        allocator_{} {
    auto buf = ReadFile(mgr, config.wfloat.model);
    Init(buf.data(), buf.size());
  }
  ...
  Ort::Value Run(Ort::Value x, Ort::Value tones, int64_t sid, float speed) {
    ...
    float noise_scale = config_.wfloat.noise_scale;
    float length_scale = config_.wfloat.length_scale;
    float noise_scale_w = config_.wfloat.noise_scale_w;
  ...
  const OfflineTtsWfloatModelMetaData &GetMetaData() const { return meta_data_; }
 ...
 private:
  ...
  OfflineTtsWfloatModelMetaData meta_data_;
};

OfflineTtsWfloatModel::OfflineTtsWfloatModel(const OfflineTtsModelConfig &config)
    : impl_(std::make_unique<Impl>(config)) {}

template <typename Manager>
OfflineTtsWfloatModel::OfflineTtsWfloatModel(Manager *mgr,
                                         const OfflineTtsModelConfig &config)
    : impl_(std::make_unique<Impl>(mgr, config)) {}

OfflineTtsWfloatModel::~OfflineTtsWfloatModel() = default;

Ort::Value OfflineTtsWfloatModel::Run(Ort::Value x, int64_t sid /*=0*/,
                                    float speed /*= 1.0*/) {
  return impl_->Run(std::move(x), sid, speed);
}

Ort::Value OfflineTtsWfloatModel::Run(Ort::Value x, Ort::Value tones,
                                    int64_t sid /*= 0*/,
                                    float speed /*= 1.0*/) const {
  return impl_->Run(std::move(x), std::move(tones), sid, speed);
}

const OfflineTtsWfloatModelMetaData &OfflineTtsWfloatModel::GetMetaData()
    const {
  return impl_->GetMetaData();
}

#if __ANDROID_API__ >= 9
template OfflineTtsWfloatModel::OfflineTtsWfloatModel(
